...
def get_commits_since_last_tag() -> list[str]:
    ...
    try:
        ...
        last_tag = subprocess.check_output(
            ["git", "describe", "--tags", "--abbrev=0", "HEAD^"], stderr=subprocess.DEVNULL, text=True
        ).strip()
    ...
    # Get commits
    commits = (
        subprocess.check_output(["git", "log", commit_range, "--pretty=format:%H|%s|%b"], text=True).strip().split("\n")
    )
